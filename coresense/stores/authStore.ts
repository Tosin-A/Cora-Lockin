...
async function initializeUserForNewSignup(userId: string, email: string, fullName?: string) {
  try {
    ...
    const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/user/initialize`, {
      method: 'POST',
      headers: {
    ...
    const usageError = !response.ok ? new Error(`HTTP ${response.status}`) : null;
  ...
  checkAuth: async () => {
    try {
      ...
      if (session?.user) {
        ...
        if (currentUser?.id === session.user.id && currentState.isAuthenticated) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('Invalid login credentials')) {
          throw new Error('Invalid email or password. Please check your credentials or use "Sign Up" to create an account.');
        }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('already registered')) {
          throw new Error('An account with this email already exists. Please use "Sign In" or try a different email.');
        }
      ...
      try {
        await initializeUserForNewSignup(data.user.id, data.user.email || '', fullName);
      } catch (initError) {
      ...
      if (!data.user.email_confirmed_at) {
        set({ isLoading: false });
        const confirmationError: any = new Error('Account created! Please check your email to confirm your account before signing in.');
